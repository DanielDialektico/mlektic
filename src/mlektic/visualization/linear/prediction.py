...
def explain_lr_prediction(
    ...
    *,
    ...
    # Formatting helper to avoid exploding values escaping the box
    def _fmt(val):
        s = f"{float(val):.{dec}f}"
    ...
    # Theme config for custom elements
    p = _resolve(theme)
    ...
    def custom_updatemenus(buttons, x=0.08, y=1.10):
        return [dict(
            type="buttons",
            direction="left",
            x=x, y=y,
    ...
    def custom_sliders(steps):
        cv = dict(prefix="Stage: ")
        if p.get("slider_font_color"):
    ...
    if d == 1:
        ...
        vars_tex = r"$\begin{aligned}" + rf"x_1 &= {_fmt(xq1_disp)}" + r"\end{aligned}$"
        subst_tex = (
            r"$\begin{aligned}"
        ...
        res_tex = (
            r"$\begin{aligned}"
            rf"\hat{{y}} &= {_fmt(yhat)}\\"
            rf"(x_1, \hat{{y}}) &= ({_fmt(xq1_disp)}, {_fmt(yhat)})"
            r"\end{aligned}$"
        )
        ...
        fig.update_layout(
            **layout_kwargs,
            ...
            updatemenus=custom_updatemenus(buttons),
    ...
    if d == 2:
        ...
        vars_tex = r"$\begin{aligned}" + rf"&x_1 = {_fmt(xq1_disp)}\\" + rf"&x_2 = {_fmt(xq2_disp)}" + r"\end{aligned}$"
        subst_tex = (
            r"$\begin{aligned}"
        ...
        res_tex = (
            r"$\begin{aligned}"
            rf"\hat{{y}} &= {_fmt(yhat)}\\"
            rf"(x_1, x_2, \hat{{y}}) &= ({_fmt(xq1_disp)}, {_fmt(xq2_disp)}, {_fmt(yhat)})"
            r"\end{aligned}$"
        )
    ...
    if d <= 15:
        ...
        vars_lines = [rf"x_{{{j+1}}} = {_fmt(x_disp[j])}" for j in range(d)]
        vars_tex = r"$\begin{aligned}" + r"\\ ".join([rf"&{ln}" for ln in vars_lines]) + r"\end{aligned}$"

        subst_lines = []
        for j in range(d):
            plus = r"+\;" if j < d - 1 else r""
            subst_lines.append(rf"({_fmt(w_disp[j])})\cdot({_fmt(x_disp[j])})\;{plus}")
        subst_lines.append(rf"({_fmt(b_disp)})")
        ...
        fig.update_layout(
            **layout_kwargs, shapes=shapes, annotations=ann_slots(0),
            updatemenus=custom_updatemenus(buttons), sliders=custom_sliders(slider_steps),
        )
        return fig
    ...
    x_items = [rf"{_fmt(x_disp[j])}" for j in range(d)]
    ...
    th_items = [rf"{_fmt(w_disp[j])}" for j in range(d)]
    ...
    theta0_tex = rf"$\theta_0 = {_fmt(b_disp)}$"
